    ...
    def _default_generator(self) -> RawGenerator:
        session = LlamaCppGenerationSession(
            LocalGenerationConfig(
                ...
                expected_model_sha256=self._model_file_sha256,
                context_tokens=self.config.context_tokens,
                prompt_batch_tokens=self.config.prompt_batch_tokens,
                max_input_tokens=self.config.max_input_tokens,
                max_new_tokens=self.config.max_new_tokens,
            ...
            runtime_version=self._llama_cpp_python_version,
        )
        self._device_used = session.diagnostics.device
        self._effective_gpu_layers = session.diagnostics.effective_gpu_layers
        self._chat_template_sha256 = session.diagnostics.chat_template_sha256

        def generate(batch: list[GenerationRequest]) -> list[GenerationResponse]:
            return session.generate(batch)
